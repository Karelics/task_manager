...
class MissionUnittest(unittest.TestCase):
    ...
    def test_get_current_subtask_id_tracks_progress_and_clears_after(self):
        ...
        def fake_execute_task_cb(_goal, _goal_handle):
            seen_subtask_ids.append(self.mission.get_current_subtask_id(goal_id))
        ...
        self.assertIsNone(self.mission.get_current_subtask_id(goal_id))
        self.mission.execute_cb(goal_handle=Mock(request=request, goal_id=Mock(uuid=[0] * 16)))

        self.assertEqual(seen_subtask_ids, ["a", "b"])
        self.assertIsNone(self.mission.get_current_subtask_id(goal_id))

    def test_get_current_subtask_id_is_independent_per_mission_invocation(self):
        ...
        def fake_execute_task_cb(goal, _goal_handle):
            if goal.task_id == "a":
                ...
                self.assertEqual(self.mission.get_current_subtask_id(goal_id_a), "a")
            return ExecuteTask.Result(task_status=TaskStatus.DONE, task_result="{}")

        self.mission.execute_task_cb.side_effect = fake_execute_task_cb
        self.mission.execute_cb(goal_handle=Mock(request=request_a, goal_id=Mock(uuid=list(goal_id_a))))

        self.assertIsNone(self.mission.get_current_subtask_id(goal_id_a))
        self.assertIsNone(self.mission.get_current_subtask_id(goal_id_b))
